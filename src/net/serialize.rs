...
pub async fn read_str<R: AsyncReadExt + std::marker::Unpin>(
    reader: &mut R,
    max_len: i16,
) -> Result<String, Error> {
    ...
    if len < 0 || len > max_len {
        return Err(Error::new(
            ...
            format!("string length {} is not within bounds 0..={}", len, max_len),
        ));
    }
    ...
    pub async fn read<R: AsyncReadExt + std::marker::Unpin>(
        ...
        Ok(match ty {
            ...
            other => {
                return Err(Error::new(
                    ErrorKind::InvalidInput,
                    format!("entity data type {} is not supported", other),
                ))
            }
        })
    }
